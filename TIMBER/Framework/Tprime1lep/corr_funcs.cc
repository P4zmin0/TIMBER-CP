...
// IDK what this function does. What's HLT? seems to be calling a bunch of variables under HLT (electrons? and a muon correction)
// I assumed hltfunc is a double type vector because of the definition of its return value below.
RVec<double> hltfunc(vector<float> &elhlt_pts, vector<float> &elhlt_etas, vector<vector<float>> &elechltsfs, vector<vector<float>> &elechltuncs, string &yrstr, const float &pt, const float &eta, const bool &isEl)
